...
playerApp.controller('Player', function($scope,$http,$routeParams,$timeout,charts){
	...
	$http(request).then(function(player){
		...
		$scope.gmYdsData = [$scope.pass.plus300,$scope.pass.plus200,$scope.pass.plus100,$scope.pass.under100];
...
playerApp.controller('Compare', function($scope,$http,$routeParams,$timeout,charts,$parse){
	...
	$scope.reqPlayer = function(n){
		...
		$http(request).then(function(player){
			...
			$timeout(function(){charts.updateCareerCompareChart($scope.players, $scope.chartStat, $scope.totalYears)});
		...
		$timeout(function(){
			$(".player-select .info").each(function(index,e){
				if ($(this).position().top > 73){
					...
					$(this).find("p:first-child").css("font-size","8.5px");
				}
			})
		},400);
	...
	$scope.erasePlayer = function(n){
		...
		charts.updateCareerCompareChart($scope.players, $scope.chartStat, $scope.totalYears);
		...
		angular.forEach($scope.players,function(player,index){
			if (player.hasOwnProperty("stats")){
				player.stats.allYears.forEach(function(year){
					if (!$scope.totalYears.includes(year)){
						$scope.totalYears.push(year);
					}
				})
			$scope.totalYears = $scope.totalYears.sort();
			}
		})
	}
	...
	//PLAYER STATS SCREEN CHARTS
		...
		//Games over 300 yds, etc
			...
			window.gmYdsChart = new Chart(ctx, {
				type: "doughnut",
				data: {
					labels: [" 300+ Yards", " 200-299 Yards", " 100-199 Yards", " >100 Yards"],
					datasets: [{
						data: data,
						backgroundColor: colors
					}],
